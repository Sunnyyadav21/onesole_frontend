...
const VitalChart = () => {
    ...
    const handleFormSubmit = (event) => {
        ...
        if (form.checkValidity() === false) {
            ...
            const payload = medicines.map((medicine) => ({
                ...
                time: medicine.time,
                vitals: Object.entries(medicine.vitals).map(([vitalEntryId, value]) => ({
                    vitalEntryId,
                    value
                }))
            }));

            fetch('http://localhost:3000/v1/vital-charts/bulk', {
                method: 'POST',
                headers: {
    ...
    useEffect(() => {
        fetch('http://localhost:3000/v1/vital-entries?status=active', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                   Authorization: `Bearer ${tokens}`
    ...
    return (
        <div className='col-md-12 pt-4'>
            <Card>
                <Card.Header>
                    <h4 className="header-title">Add Vital</h4>
                </Card.Header>
                <Card.Body>
                    <Row>
                        <Col lg={4}>
