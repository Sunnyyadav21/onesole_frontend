...
const ProfileDropdown = ({
	...
	useEffect(() => {
		getUserDetail(localStorage.getItem('user-details'))
		setrefreshTokens(localStorage.getItem('refresh-tokens'))
	})


	const logout = () => {
		axios.post('http://localhost:3000/v1/auth/logout', {
			...
			>
				...
				<span className="d-lg-block d-none">
					<h5 className="my-0 fw-normal">
						{userDetail && JSON.parse(userDetail).name}
			...
			>
				<div onClick={toggleDropdown}>
					<div className=" dropdown-header noti-title">
						<h6 className="text-overflow m-0">Welcome {userDetail && JSON.parse(userDetail).name}</h6>
					</div>
					{/* item*/}

					{(menuItems || []).map((item, idx) => {
						return (
							<Link key={idx} to={item.redirectTo} className="dropdown-item" onClick={logout}>
