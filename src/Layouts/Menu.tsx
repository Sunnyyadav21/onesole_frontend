...
import { role } from '@/pages/ui/forms/data'
...
const MenuItemWithChildren = ({
	...
	return (
		<li className={`side-nav-item test ${open ? 'menuitem-active' : ''}`}>

			{role && JSON.parse(role).role == 'user' && item.key != "hospital" && item.key != "authProtectedRoutes" && item.key != "Admin Management" && item.key != 'Patient' ? <Link
				to="#"
			...
			</Link> : role && JSON.parse(role).role == 'admin' ? <Link
				to="#"
				className={`side-nav-link ${linkClassName} ${activeMenuItems!.includes(item.key) ? 'open' : ''
					}`}
				aria-expanded={open}
...
const MenuItem = ({ item, className, linkClassName }: SubMenus) => {
	...
	if (role && JSON.parse(role).role == 'user') {
		return (
			<li className={`side-nav-item ${className}`}>
				<MenuItemLink item={item} className={linkClassName} />
			</li>
		)
	} else if (role && JSON.parse(role).role == 'admin') {
		return (
			<li className={`side-nav-item ${className}`}>
				<MenuItemLink item={item} className={linkClassName} />
			</li>
...
const MenuItemLink = ({ item, className }: SubMenus) => {
	...
	return (
		role && JSON.parse(role).role == 'user' && item.key != "Add Doctor" && item.key != "All Doctor" && item.key != "Add User" && item.key != "Add Medicine" && item.key != "Add Vital Chart 2" && item.key != "All Vital"

			&& item.key != "All Vital" && item.key != "Add Vital Chart" && item.key != "Add Vital" && item.key != "Add Diet Chart" && item.key != "Add Diet" && item.key != "All Diet" && item.key != "Add Report" ? <Link
				to={'/medicalapp' + item.url!}
				target={item.target}
				className={`side-nav-link-ref dfgdgd ${className}`}
				data-menu-key={item.key}
			>
			{item.icon && <i className={item.icon} />}
			{item.badge && (
